...
" ##### START HERE #######################################################
"
" Detect ingrid file and treat appropriately
"
"au BufNewFile,BufRead *.tg setf ingrid
"
" #####  END HERE  #######################################################
"
" In addition, this file needs to be in the right location, as follows:
"
" unix: $HOME/.vim/syntax/ingrid.vim
"       $VIMRUNTIME/syntax/ingrid.vim
"
" windows: c:\vim\vimfiles\syntax\ingrid.vim, or
"          $VIMRUNTIME\syntax\ingrid.vim
"
...
syn keyword tgConOut sami starcd save write
...
syn keyword tgConAnaSub acrx anal auto ratl cost mxnre opnit mnss mxss itewin
syn keyword tgConAnaSub iplt iprt sw3 sw6 sw7 accflg maxaug igapfg itpro neig shift nip1 nip2
syn keyword tgConAnaSub nbei nibsr msrf dctol ectol retol lstol
...
syn keyword tgConMat marcmats mate mt mti shelli
syn keyword tgConMat nastmats nensmats
syn keyword tgConMat patmats
syn keyword tgConMat rho contained
syn keyword tgConMat shso shelli
syn keyword tgConMat tmm tz3dmats


syn keyword tgConMatSub beta brick contained
syn keyword tgConMatSub csb contained
syn keyword tgConMatSub e efp eh elfob elfor elp etan contained
...
syn keyword tgConMatSub sths tthi contained
syn keyword tgConMatSub mrt contained
syn keyword tgConMatSub xtrans ytrans ztrans xrot yrot zrot comflg xcom ycom zcom contained
syn keyword tgConMatSub p31 p32 p33 p34 p35 p36 p37 p38 contained
syn keyword tgConMatSub p41 p42 p43 p44 p45 p46 p47 p48 contained
syn keyword tgConMatSub p51 p52 p53 p54 p55 p56 p57 p58 contained
syn keyword tgConMatSub p61 p62 p63 p64 p65 p66 p67 p68 contained
syn keyword tgConMatSub p71 p72 p73 p74 p75 p76 p77 p78 contained
syn keyword tgConMatSub p81 p82 p83 p84 p85 p86 p87 p88 contained
...
syn keyword tgConInt siinfo


syn keyword tgConIntSub a3 a5 a10 a13 auto altoln altolt
syn keyword tgConIntSub break btime
syn keyword tgConIntSub concon convert18_17_dr
syn keyword tgConIntSub dcoef decay distoc dni dnt dtime dummy dx_tol
syn keyword tgConIntSub edge
syn keyword tgConIntSub fric
syn keyword tgConIntSub gcurve
syn keyword tgConIntSub i2d3d incmst incslv igap ignore isrch isym iaug icurve
syn keyword tgConIntSub kfric
syn keyword tgConIntSub lagop lag_tol like_nike3d
syn keyword tgConIntSub maxpcss mfif mortar mortar18
syn keyword tgConIntSub ncybs ncyup nsw no_rel
syn keyword tgConIntSub owsv
syn keyword tgConIntSub p1 pen penmax pnlt pnltm pnlts
syn keyword tgConIntSub rebar relocate radcon
syn keyword tgConIntSub sand scoef scoufsf sets sfif sfmps sfsps sfactor
syn keyword tgConIntSub single sl sldthk sldstf softc srchdp ssoftc shiftns
syn keyword tgConIntSub sthms sthss sv svfsf
syn keyword tgConIntSub tied thkopt thms thss type16 type17 type18
...
syn keyword tgConMerge merge write mns
...
syn keyword tgCon2DCSub lep lint lnof lo lod lp2 lpil lp lad ltp lar
...
syn keyword tgConSets fset
syn keyword tgConSets fseti
...
syn keyword tgBlkMesh res resi as asi das dasi drs drsi nds ndsi hyr relax relaxi
syn keyword tgBlkMesh splint sf sfv sfvi sfi spp
syn keyword tgBlkMesh t1 t2 t3 tr tri tf tfi tme tmei tmplt trgi
...
syn keyword tgBlkInt si sii siinfo
...
syn keyword tgMrgGraphSub marker mcolor mdep mom
...
syn keyword tgMrgGraphSub sax sc scol scolor scon crz sd sdedge sdpt sfb si size
...
syn keyword tgMrgGraphSub mat
syn keyword tgMrgGraphSub sd sp cy plan pr er cone cn2p ts cr crx cry crz cp
syn keyword tgMrgGraphSub xyplan yzplan zxplan xzplan sds xcy ycy zcy pl3 iplane
syn keyword tgMrgGraphSub ppx ppy ppz cnsp cncy pon pox poy poz
...
syn keyword tgConditional if then elseif else endif while endwhile for endfor when elsewhen endwhen
...
syn match tgMathOps "!="
syn match tgMathOps "sin("
syn match tgMathOps "asin("
syn match tgMathOps "cos("
syn match tgMathOps "acos("
syn match tgMathOps "tan("
syn match tgMathOps "atan("
syn match tgMathOps "sqrt("
...
syn region tgString start="sititle" end="$"

syn keyword tgInterrupt interrupt
...
syn region tgBlkForce start="\(nikemats\|dynamats\|lsdymats\)" end=";" contains=@tgGenGroup,tgConMatSub,tgBlkForce2
syn region tgBlkForce start="\(eps\|es\)" end=";" contains=@tgGenGroup,tgConMatSub
...
" "para" command
syn region Normal matchgroup=tgComplex start="para" end=";" contains=tgNumber,tgIdentifier,tgMathOps,tgComment,tgConditional
" "bi" command
...
syn region Normal matchgroup=tgComplex2  start="include"        end="$" contains=tgComment,tgConditional keepend
syn region Normal matchgroup=tgComplex2  start="cubit"          end="$" contains=tgComment,tgConditional
syn region Normal matchgroup=tgComplex2  start="printf"         end=";"
syn region Normal matchgroup=tgComplex2  start="slice_model"    end=";" contains=tgNumber,tgIdentifier,tgMathOps,tgComment,tgConditional
syn region Normal matchgroup=tgComplex2  start="comment"        end="$" contains=tgNumber,tgIdentifier,tgMathOps,tgComment,tgConditional,tgConIntSub keepend
syn region Normal matchgroup=tgComplex2  start="sid"            end=";" contains=tgNumber,tgIdentifier,tgMathOps,tgComment,tgConditional,tgConIntSub,tgString keepend
syn region Normal matchgroup=tgComplex2  start="input"          end="=" contains=tgNumber,tgIdentifier,tgMathOps,tgComment,tgConditional,tgConIntSub,tgString keepend
syn region Normal matchgroup=tgComplex2  start="user_load"      end=";" contains=tgComment,tgConditional keepend
...
if version >= 508 || !exists("did_tg_syntax_inits")
    ...
    HiLink tgComplex2       Statement
    HiLink tgBoolean        Boolean
    HiLink tgString         Error
    HiLink tgInterrupt      Error
...
" even though it is a bit slower, ingrid files are not typically too
" large, so it should not be a problem.
"
syn sync fromstart


let b:current_syntax = "ingrid"

"
" Using plug-in matchit.vim, extend matching with % to (if, else, elseif and endif) and (while endwhile)
"
let b:match_words = '\<if\>:\<then\>:\<else\>:\<elseif\>:\<endif\>,'
\ . '\<while\>:\<endwhile\>:\<for\>:\<endfor\>:\<when\>:\<elsewhen\>:\<endwhen\>'
